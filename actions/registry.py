...
ACTION_PACKAGE = os.path.dirname(__file__)
...
def action(
    ...
    def decorator(func):
        ...
        param_names = set(
            [ActionParameter.model_validate(param).name for param in parameters]
        )
...
class ActionRegister:
    def __init__(self, agent, action_path: str) -> None:
        self.abilities = {}
        self.register_abilities(action_path)
        self.agent = agent

    def register_abilities(self, action_name) -> None:
        action_path = os.path.join(ACTION_PACKAGE, f"{action_name}.py")
        ...
        if not os.path.basename(action_path) in [
            ...
            action = os.path.relpath(
                action_path, ACTION_PACKAGE
            ).replace("/", ".")
            try:
                module = importlib.import_module(
                    f".{action[:-3]}", package=ACTION_PACKAGE
                )
